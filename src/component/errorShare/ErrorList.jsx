...
import { getErrorList } from "../../store/notification/notificationSlice";
    ...
    <div className="table items-center w-1/2 h-5/6 min-w-40 min-h-40 my-20 mx-20 border-4 border-yellow-300 rounded-3xl">
      ...
      {status === "successed" && (
        <div className="flex flex-col items-center w-full h-5/6 mt-10">
          <div className="text-center font-bold text-3xl">에러 공유</div>
          <ul role="list" className="divide-y divide-gray-100">
            {data.map((el) => (
              <li key={el.id} className="flex justify-between gap-x-6 py-5">
                <div className="flex gap-x-4">
                  <div className="min-w-0 flex-auto">
                    <Link to={`/community/error/${selectData}/${el.id}`}>
