...
const portfoilo = [
  {
    name: "수업노트",
    description: "나만의 수업노트",
    href: "/portfolio/note",
    icon: DocumentTextIcon,
  },
  {
    name: "스터디",
    description: "활발한 스터디는 개발자의 원동력",
    href: "/portfolio/study",
    icon: UsersIcon,
  },
  {
    name: "프로젝트",
    description: "부트캠프의 산출물",
    ...
    <header className="bg-white border-b-2 border-yellow-300 shadow-sm">
      ...
      >
        ...
        <Popover.Group className="hidden lg:flex lg:gap-x-12">
          ...
          <Popover className="relative">
            <Popover.Button className="flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900">
              <Link to={"/portfolio/note"}>포트폴리오</Link>
              <ChevronDownIcon
                className="h-5 w-5 flex-none text-gray-400"
                aria-hidden="true"
              />
        ...
        {data.userName ? (
          <div className="hidden lg:flex lg:flex-1 lg:justify-end font-semibold">
            <div>
              <Link to={"/my"}>{data.userName}님</Link>
            </div>
            <button
              className="ml-3"
              onClick={() => {
